# AGENTS.md

## Cursor Cloud specific instructions

- **Stack**: Next.js 16 (App Router, static export), React 19, TypeScript 5, Tailwind CSS 4, shadcn/ui. Fully client-side — no backend, no database.
- **Package manager**: npm (`package-lock.json`). Run `npm install` to restore dependencies.
- **Dev server**: `npm run dev` starts on `localhost:3000`.
- **Scripts**: `npm run lint` (ESLint), `npm run test` (Vitest), `npm run build` (static export).
- **Pre-existing lint errors**: The repo has ~9 `react-hooks/set-state-in-effect` errors and a few `@next/next/no-img-element` warnings. These are pre-existing and not caused by agent changes.
- **No env vars required**: Optional vars in `.env.example` — `NEXT_PUBLIC_SITE_URL` (defaults to `https://tukarqr.my`), `NEXT_PUBLIC_DISCORD_RATING_WEBHOOK_URL`, `NEXT_PUBLIC_GA_MEASUREMENT_ID`.
- **Static export**: `next build` produces a fully static site (`output: "export"` in `next.config.ts`). No server runtime needed.
- **MCP (Workers)**: Remote MCP at `https://mcp.tukarqr.my/mcp`. Package `mcp/` is a Cloudflare Worker (`tukarqr-mcp`), not Pages. Scripts: `npm run mcp:dev`, `npm run mcp:deploy`. Shared QR logic lives in `packages/qr-core`. Agent skill: `mcp/SKILL.md` and `.agents/skills/tukarqr-mcp/SKILL.md`. Tools: validate, parse, details, decode (PNG/JPEG), styled encode, photo convert, bulk ZIP. Exports match the about-page `comparison-after.png` Malaysia National QR card — no DuitNow logo. Default size is square `1:1`; `3:4` when requested.
