...
- **MCP (Workers)**: Remote MCP at `https://mcp.tukarqr.my/mcp`. Package `mcp/` is a Cloudflare Worker (`tukarqr-mcp`), not Pages. Scripts: `npm run mcp:dev`, `npm run mcp:deploy`. Shared QR logic lives in `packages/qr-core`. Tools are text-only (`get_validate_duitnow_qr`, `get_parse_duitnow_qr`, `get_encode_qr`).
